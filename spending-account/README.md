...
A treasury contract that holds a large NEAR balance (and optionally NEP-141 token balances) and pays it out only to whitelisted accounts, each capped by a spending limit. Control is split between three accounts, each expected to be a [Sputnik DAO](https://github.com/near-daos/sputnik-dao-contract), either acting through its own [`dao-timelock`](../dao-timelock) or holding the role directly.

## Design

- **Three separated roles.** The **spender** can only transfer NEAR or whitelisted NEP-141 tokens to already-whitelisted accounts within their remaining allowance — nothing else. The **manager** manages the whitelist and the limits — but cannot transfer funds. The **admin** only assigns the roles (`set_spender`, `set_manager`, `set_admin`). No single role can both whitelist an account and pay it. Whether a role's actions are additionally delayed depends on the deployment: in the HoS topology the admin and manager act through a timelock where guardians can cancel their requests, while the spender of the Surplus Spending Account is a DAO holding the role directly — its payouts are bounded by the whitelist and the limits alone.
